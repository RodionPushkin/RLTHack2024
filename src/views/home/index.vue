<template>
  <div class="container">
    <div class="side" v-auto-animate>
      <div class="fixed">
        <div class="search" v-auto-animate>
        ...
        <div class="search" v-auto-animate>
          <div class="search-bar">
            <input
              maxlength="200"
              placeholder="поиск по коду закупки"
              type="text"
              v-model="code"
              @keyup="request"
              @keyup.enter="request"
        ...
        <div class="date" v-auto-animate>
          <label for="date" v-if="!filters.date.useRange">Дата</label>
          <div v-if="!filters.date.useRange">
            <input
              type="date"
              id="date"
          ...
          <label for="dateFrom" v-if="filters.date.useRange">Дата c</label>
          <div v-if="filters.date.useRange">
            <input
              type="date"
              id="dateFrom"
          ...
          <label for="dateTo" v-if="filters.date.useRange">Дата по</label>
          <div v-if="filters.date.useRange">
            <input
              type="date"
              id="dateTo"
        ...
        <div class="input-checkbox">
          <input
            type="checkbox"
            v-model="filters.isSupplierDefined"
            id="isSupplierDefined"
            @change="request"
          />
          <label for="isSupplierDefined"></label>
          <label for="isSupplierDefined"></label>
          определен ли поставщик?
        </div>
        <div class="region">
        ...
        <div class="search" v-auto-animate>
          <div class="search-bar">
            <input
              maxlength="200"
              placeholder="поиск по ОКПД2"
              type="text"
              v-model="filters.okpd2.value"
              @keyup="request"
              @keyup.enter="request"
            />
            <svg
  ...
  methods: {
    request() {
      ...
      this.timer = setTimeout(() => {
        const body = {};
        if (this.code.length > 0) body.code = this.code;
        body.small = true;
        this.$api
          .post("search_zakupka", undefined, body)
          .then((res) => {
            if (res.data) {
              this.data = [];
        ...
        //   const date = this.filters.date.from.split("-");
        //   console.log(`${date[2]}.${date[1]}.${date[0]}`);
        // } else {
        //   console.log(this.filters.date.from);
        // }
      }, 700);
    },
  },
